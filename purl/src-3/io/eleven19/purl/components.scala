...
object components {
  ...
  object Subpath extends NewtypeCustom[Option[String]] {
    val none: Subpath = wrap(None)

    protected def validate(value: Option[String]) =
      validators.SubpathValidator.validate(value.orNull)

    protected inline def validateInline(inline value: Option[String]) = 
      ${ validators.SubpathValidator.validateInlineImpl('{value.orNull}) }
